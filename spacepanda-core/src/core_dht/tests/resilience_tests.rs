...
use crate::core_dht::{
    DhtCommand, DhtConfig, DhtEvent, DhtKey, DhtNode, DhtStorage, DhtValue, PeerContact,
    RoutingTable,
};
...
async fn test_dht_provider_expiration() {
    ...
    assert!(
        expired_result.unwrap_err().contains("expired"),
...
async fn test_dht_malicious_peer_handling() {
    ...
    assert!(
        closest.iter().any(|p| p.id == malicious_id),
        "Malicious peer should be in routing table"
    );
    ...
    assert!(
        !after_cleanup.iter().any(|p| p.id == malicious_id),
        "Malicious peer should be removed"
    );
...
async fn test_dht_deep_routing_correctness() {
    ...
    println!(
        "✅ Deep routing correctness: XOR distance routing verified with {} peers",
        all_peers_in_table.len()
    );
}
...
async fn test_dht_routing_table_consistency() {
    ...
    assert!(
        !all_peers.iter().any(|p| p.id == local_id),
        "Local node should not be in routing table"
    );

    println!(
        "✅ Routing table consistency: {} unique peers after concurrent operations",
        all_peers.len()
    );
}
