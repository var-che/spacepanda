...
impl DhtServer {
    ...
    async fn handle_find_node(&self, _from: DhtKey, target: DhtKey, _request_id: u64) -> Result<(), String> {
        ...
        let _nodes: Vec<PeerInfo> = closest.iter()
    ...
    async fn handle_find_value(&self, _from: DhtKey, key: DhtKey, _request_id: u64) -> Result<(), String> {
        // Try to get value from local storage
        let _result = match self.storage.get(&key) {
            Ok(value) => {
                // Value found, emit event
                let _ = self.event_tx.send(DhtEvent::ValueFound {
                    key,
                    value: value.clone(),
                }).await;
            ...
            Err(_) => {
                ...
                let nodes: Vec<PeerInfo> = closest.iter()
                    .map(|contact| PeerInfo::new(contact.id, contact.address.clone()))
                    .collect();
    ...
    async fn handle_store(&self, _from: DhtKey, key: DhtKey, value: DhtValue, _request_id: u64) -> Result<(), String> {
        // Validate value
        ...
        match validation_result {
            ...
            Err(e) => {
                // Validation failed
                let _ = self.event_tx.send(DhtEvent::ValidationFailed {
                    key,
                    reason: e.clone(),
                }).await;
                Err(e)
...
mod tests {
    ...
    async fn test_handle_find_node() {
        ...
        let _ = table.insert(PeerContact::new(DhtKey::hash(b"peer1"), "127.0.0.1:8001".to_string()));
        let _ = table.insert(PeerContact::new(DhtKey::hash(b"peer2"), "127.0.0.1:8002".to_string()));
        let _ = table.insert(PeerContact::new(DhtKey::hash(b"peer3"), "127.0.0.1:8003".to_string()));
        drop(table);
