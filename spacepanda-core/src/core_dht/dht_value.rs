...
impl DhtValue {
    ...
    pub fn validate(&self, max_size: usize, require_signature: bool) -> Result<(), String> {
        ...
        if self.data.len() > max_size {
            return Err(format!("Value size {} exceeds maximum {}", self.data.len(), max_size));
        }
...
mod tests {
    ...
    fn test_dht_value_with_ttl_duration() {
        let value = DhtValue::new(vec![1, 2, 3]).with_ttl_duration(Duration::from_secs(7200));
    ...
    fn test_dht_value_with_signature() {
        ...
        let value = DhtValue::new(vec![1, 2, 3]).with_signature(signature.clone());
    ...
    fn test_dht_value_validate_with_signature() {
        let value = DhtValue::new(vec![1, 2, 3]).with_signature(vec![0xAA, 0xBB]);
        let result = value.validate(1024, true);
    ...
    fn test_dht_value_with_crdt_metadata() {
        ...
        let value = DhtValue::new(vec![1, 2, 3]).with_crdt_metadata(metadata.clone());
