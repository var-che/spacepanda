...
impl BucketEntry {
    fn new(node_id: DhtKey) -> Self {
        ...
        BucketEntry { node_id, last_seen: now, rtt_ms: None }
    }
...
impl RoutingTable {
    fn new(local_id: DhtKey, bucket_size: usize, num_buckets: usize) -> Self {
        RoutingTable { local_id, buckets: vec![Vec::new(); num_buckets], bucket_size }
    }
...
pub enum DhtCommand {
    ...
    Put { key: DhtKey, value: DhtValue, response_tx: oneshot::Sender<Result<(), String>> },
    /// Get a value from the DHT
    Get { key: DhtKey, response_tx: oneshot::Sender<Result<Option<DhtValue>, String>> },
    /// Find nodes closest to a key
    FindNode { target: DhtKey, response_tx: oneshot::Sender<Result<Vec<DhtKey>, String>> },
    /// Handle incoming DHT message
    HandleMessage { from: DhtKey, message: DhtMessage },
    /// Bootstrap with a known node
    Bootstrap { node_id: DhtKey },
...
impl DhtNode {
    ...
    pub fn new(
        ...
        Ok(DhtNode { config, local_id, routing_table, storage, event_tx })
    }
    ...
    pub async fn run(self: Arc<Self>, mut command_rx: mpsc::Receiver<DhtCommand>) {
    ...
    async fn handle_get(&self, key: DhtKey) -> Result<Option<DhtValue>, String> {
        ...
        let _ = self
            .event_tx
            .send(DhtEvent::SearchFailed { key, reason: "Value not found locally".to_string() })
            .await;
    ...
    async fn handle_find_node(&self, target: DhtKey) -> Result<Vec<DhtKey>, String> {
        Ok(self.routing_table.lock().await.find_closest(&target, self.config.bucket_size))
    }
    ...
    async fn handle_message(&self, from: DhtKey, message: DhtMessage) {
        ...
        match message {
            ...
            DhtMessage::FindNode { target, .. } => {
                let _closest =
                    self.routing_table.lock().await.find_closest(&target, self.config.bucket_size);
                // TODO: Send FindNodeResponse via router
            }
            DhtMessage::Store { key, value, .. } => {
                // Validate and store
                if value
                    .validate(self.config.max_value_size, self.config.require_signatures)
                    .is_ok()
                {
            ...
            DhtMessage::GetValue { key, .. } => {
                ...
                let _closest =
                    self.routing_table.lock().await.find_closest(&key, self.config.bucket_size);
    ...
    async fn handle_bootstrap(&self, node_id: DhtKey) {
        ...
        let _ = self.event_tx.send(DhtEvent::NodeDiscovered { node_id }).await;
    }
...
mod tests {
    ...
    async fn test_dht_node_put_get() {
        ...
        cmd_tx
            .send(DhtCommand::Put { key, value: value.clone(), response_tx })
            .await
            .unwrap();
        ...
        let _node_discovered =
            tokio::time::timeout(Duration::from_millis(100), event_rx.recv()).await.unwrap();
        ...
        // Get the value
        let (response_tx, response_rx) = oneshot::channel();
        cmd_tx.send(DhtCommand::Get { key, response_tx }).await.unwrap();
    ...
    async fn test_dht_node_find_node() {
        ...
        for i in 0..5 {
            cmd_tx
                .send(DhtCommand::Bootstrap { node_id: DhtKey::hash_string(&format!("node{}", i)) })
                .await
                .unwrap();
        }
        ...
        let target = DhtKey::hash_string("target");
        let (response_tx, response_rx) = oneshot::channel();
        cmd_tx.send(DhtCommand::FindNode { target, response_tx }).await.unwrap();
    ...
    async fn test_dht_node_routing_table_stats() {
        ...
        for i in 0..5 {
            node.routing_table
                .lock()
                .await
                .add_node(DhtKey::hash_string(&format!("node{}", i)));
        }
    ...
    fn test_dht_message_serialization() {
        let msg = DhtMessage::Ping { sender_id: DhtKey::hash_string("sender") };
