...
fn derive_key_from_passphrase(passphrase: &str, salt: &[u8]) -> MlsResult<Zeroizing<Vec<u8>>> {
    let params = Params::new(ARGON2_MEM_COST, ARGON2_TIME_COST, ARGON2_PARALLELISM, Some(KEY_SIZE))
    ...
    let hash_bytes =
        hash.hash.ok_or_else(|| MlsError::CryptoError("No hash output".to_string()))?;
...
pub fn encrypt_group_state(
    ...
    let (key_bytes, salt) = if let Some(pass) = passphrase {
        ...
        rand::rng().fill_bytes(&mut salt_vec);
        ...
        rand::rng().fill_bytes(&mut key_vec);
    ...
    Ok(EncryptedGroupBlob { header, nonce: nonce_bytes.to_vec(), ciphertext })
}
...
pub fn decrypt_group_state(
    ...
    let key_bytes = if let Some(pass) = passphrase {
        let salt =
            blob.header.salt.as_ref().ok_or_else(|| {
                MlsError::InvalidMessage("Missing salt for passphrase".to_string())
            })?;
    ...
    let plaintext = cipher
        .decrypt(nonce, aes_gcm::aead::Payload { msg: &blob.ciphertext, aad: &aad })
        .map_err(|e| {
            MlsError::VerifyFailed(format!("Decryption failed (corrupted or wrong key): {}", e))
        })?;
