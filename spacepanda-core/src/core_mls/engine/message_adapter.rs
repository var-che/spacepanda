...
use crate::core_mls::{
    ...
    types::GroupId,
    traits::transport::{WireMessage, MessageType},
};
...
impl MessageAdapter {
    ...
    pub fn from_wire(wire: &WireMessage) -> MlsResult<MlsMessageIn> {
        MlsMessageIn::tls_deserialize_exact(&wire.payload)
            .map_err(|e| MlsError::InvalidMessage(format!("Failed to deserialize message: {:?}", e)))
    }
...
mod tests {
    ...
    fn test_wire_message_creation() {
        ...
        let wire = MessageAdapter::to_wire(
            message_bytes.clone(),
            &group_id,
            42,
