...
impl GroupOperations for OpenMlsEngine {
    ...
    async fn add_members(&self, key_packages: Vec<Vec<u8>>) -> MlsResult<Vec<u8>> {
        ...
        let parsed_packages: Vec<KeyPackage> = key_packages.iter()
            .map(|bytes| {
                // First deserialize to KeyPackageIn, then convert to KeyPackage
                let kp_in = KeyPackageIn::tls_deserialize(&mut bytes.as_slice())
                    .map_err(|e| MlsError::InvalidMessage(format!("Invalid key package: {:?}", e)))?;
                kp_in.validate(self.provider().crypto(), ProtocolVersion::default())
                    .map_err(|e| MlsError::InvalidMessage(format!("Key package validation failed: {:?}", e)))
            })
    ...
    async fn remove_members(&self, leaf_indices: Vec<u32>) -> MlsResult<Vec<u8>> {
        ...
        let indices: Vec<LeafNodeIndex> = leaf_indices.iter()
    ...
    async fn send_message(&self, plaintext: &[u8]) -> MlsResult<Vec<u8>> {
        ...
        message.tls_serialize_detached()
            .map_err(|e| MlsError::SerializationError(format!("Failed to serialize message: {:?}", e)))
    }
    ...
    async fn process_message(&self, message: &[u8]) -> MlsResult<ProcessedMessage> {
        ...
        let mls_message = MlsMessageIn::tls_deserialize_exact(message)
            .map_err(|e| MlsError::InvalidMessage(format!("Failed to parse MLS message: {:?}", e)))?;
        
        // Convert to ProtocolMessage for processing
        let protocol_message = mls_message.try_into_protocol_message()
            .map_err(|_| MlsError::InvalidMessage("Expected protocol message, got welcome or key package bundle".to_string()))?;
        ...
        let result = match processed.into_content() {
            ...
            ProcessedMessageContent::StagedCommitMessage(staged_commit) => {
                ...
                group.merge_staged_commit(self.provider(), *staged_commit)
                    .map_err(|e| MlsError::InvalidMessage(format!("Failed to merge staged commit: {:?}", e)))?;
    ...
    async fn commit_pending(&self) -> MlsResult<CommitResult> {
        ...
        // Commit pending proposals
        let (commit_msg, welcome_opt, _group_info) = group.commit_to_pending_proposals(
            self.provider(),
            self.signature_keys(),
        ).map_err(|e| MlsError::InvalidMessage(format!("Failed to commit proposals: {:?}", e)))?;
        ...
        // Serialize messages
        let commit_message = commit_msg.tls_serialize_detached()
