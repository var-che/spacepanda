...
use super::super::{
    PeerId, RpcError, RpcMessage, RpcProtocol, SessionCommand, SessionEvent, SessionManager,
...
async fn test_noise_handshake_downgrade_protection() {
    ...
    // Test 2: Malformed handshake data should be rejected
    {
        ...
        assert!(result.is_err(), "Malformed handshake should be rejected, got: {:?}", result);
    }
    ...
    // Test 3: Plaintext injection should be rejected
    {
        ...
        assert!(result.is_err(), "Plaintext injection should be rejected, got: {:?}", result);
    }
    ...
    // Test 4: Random garbage should be rejected
    {
        ...
        let result = alice.handle_transport_event(TransportEvent::Data(conn_id, garbage)).await;

        // Should reject garbage
        assert!(result.is_err(), "Random garbage should be rejected, got: {:?}", result);
    }
    ...
    // Test 5: Verify no Established event on failed handshake
    {
        ...
        // Inject invalid data
        let _ = alice.handle_transport_event(TransportEvent::Data(conn_id, vec![0; 10])).await;
...
async fn test_onion_relay_privacy() {
    use super::super::onion_router::{OnionCommand, OnionConfig, OnionEvent, OnionRouter};
    ...
    let router = Arc::new(OnionRouter::new(config.clone(), route_table.clone(), event_tx));
    ...
    for event in observed_events {
        match event {
            OnionEvent::PacketForward { next_peer, blob } => {
                ...
                assert_ne!(
                    next_peer.0, destination.0,
                    "Relay must not see final destination as next hop"
                );
            ...
            OnionEvent::CircuitBuilt { path_length } => {
                ...
                assert_eq!(
                    path_length, config.circuit_hops,
                    "Should build {}-hop circuit",
                    config.circuit_hops
                );
            }
...
async fn test_onion_path_failure_recovery() {
    use super::super::router_handle::{RouterEvent, RouterHandle};
...
async fn test_connection_flood_protection() {
    ...
    for i in 0..100 {
        ...
        let task = tokio::spawn(async move {
            ...
            let result =
                timeout(Duration::from_millis(100), handle_clone.rpc_call(peer_id, method, params))
                    .await;
