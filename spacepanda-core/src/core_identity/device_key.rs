...
impl DeviceKeyBinding {
    ...
    pub fn new(
        ...
        DeviceKeyBinding { device_id, key_version, device_public_key, master_signature }
    }
...
impl DeviceKey {
    ...
    #[deprecated(
        since = "0.2.0",
        note = "Use register_with_proof_of_possession for production"
    )]
    ...
    pub fn rotate(&mut self, master_key: &MasterKey) -> DeviceKeyBinding {
        // Archive current key
        self.archived_keys
            .insert(self.current_version, self.active_key.public_key().to_vec());
...
mod tests {
    ...
    fn test_proof_of_possession_valid() {
        ...
        let proof = DeviceKey::create_proof_of_possession(challenge, &device_keypair);
    ...
    fn test_proof_of_possession_wrong_device_key() {
        ...
        let attacker_proof =
            DeviceKey::create_proof_of_possession(challenge.clone(), &attacker_keypair);
        ...
        let registered =
            DeviceKey::register_with_proof_of_possession(&mk, &attacker_proof).unwrap();
        assert_eq!(
    ...
    fn test_proof_of_possession_forged_signature() {
        ...
        let device_keypair = Keypair::generate(KeyType::Ed25519);
        let mut proof = DeviceKey::create_proof_of_possession(challenge, &device_keypair);
    ...
    fn test_proof_of_possession_cannot_forge_for_others_key() {
        ...
        let mut forged_proof = DeviceKey::create_proof_of_possession(challenge, &attacker_keypair);
        ...
        assert!(result.is_err(), "Cannot prove possession of someone else's key");
