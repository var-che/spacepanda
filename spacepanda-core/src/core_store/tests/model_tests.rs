...
use crate::core_store::model::{Channel, Space, Role};
use crate::core_store::model::types::{
    ChannelId, SpaceId, UserId, MessageId, ChannelType, Timestamp, 
    PermissionLevel, IdentityMeta
};
use crate::core_store::crdt::{LWWRegister, VectorClock, AddId, Crdt};
    ...
    fn test_channel_metadata_converges() {
        ...
        chan_a.name.set("Hello World".to_string(), ts_a, "node_a".to_string(), vc_a.clone());
        
        let mut vc_b = VectorClock::new();
        vc_b.increment("node_b");
        let ts_b = ts_a + 1000; // Later timestamp
        chan_b.name.set("Updated Topic".to_string(), ts_b, "node_b".to_string(), vc_b.clone());
    ...
    fn test_space_roles_merge_correctly() {
        ...
        let mod_role = Role::new("Moderator".to_string(), PermissionLevel::moderator(), "node2".to_string());
        ...
        assert_eq!(
            space1.get_role("admin").unwrap().get_name(),
            Some(&"Admin".to_string())
        );
        assert_eq!(
            space1.get_role("mod").unwrap().get_name(),
    ...
    fn test_owner_always_has_admin_permissions() {
        ...
        assert_eq!(
            space.get_user_permission_level(&owner),
    ...
    fn test_non_owner_uses_role_permissions() {
        ...
        let member_role = Role::new("Member".to_string(), PermissionLevel::member(), "node1".to_string());
        ...
        assert_eq!(
            space.get_user_permission_level(&alice),
