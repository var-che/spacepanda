...
use crate::core_store::store::{CommitLog, LocalStore, LocalStoreConfig, SnapshotManager};
...
async fn test_store_snapshot_replay() {
    ...
    // Capture final state
    let final_spaces: HashMap<_, _> = space_ids
        .iter()
        .map(|id| (id.clone(), store.get_space(id).unwrap().unwrap()))
        .collect();
    let final_channels: HashMap<_, _> = channel_ids
        .iter()
    ...
    println!(
        "Loaded from snapshot: {} spaces, {} channels",
        stats.spaces_count, stats.channels_count
    );
...
async fn test_store_storage_limits_cleanup() {
    ...
    let snapshot_count = std::fs::read_dir(&snapshots_dir).unwrap().filter(|e| e.is_ok()).count();
    ...
    let remaining_count = std::fs::read_dir(&snapshots_dir).unwrap().filter(|e| e.is_ok()).count();
