...
pub enum Proposal {
    /// Add a new member
    Add { key_package: Vec<u8>, proposed_by: String },

    /// Remove a member
    Remove { removed: LeafIndex, proposed_by: String },

    /// Update own leaf key
    Update { leaf_node: Vec<u8> },

    /// Group context extension
    GroupContextExtension { extension_data: Vec<u8> },
}
...
impl MLSState {
    ...
    pub fn new(group_id: Vec<u8>, cipher_suite: CipherSuite) -> Self {
        ...
        let epoch_start = SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_secs();
    ...
    pub fn advance_epoch(
        ...
        self.epoch_start = SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_secs();
...
mod tests {
    ...
    fn test_proposals() {
        ...
        let proposal =
            Proposal::Add { key_package: vec![1, 2, 3], proposed_by: "alice".to_string() };
    ...
    fn test_advance_epoch() {
        ...
        state.advance_epoch(vec![1, 2, 3], vec![4, 5, 6], vec![7, 8, 9]);
