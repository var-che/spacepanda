...
impl DeltaDecoder {
    ...
    pub fn decode(bytes: &[u8]) -> StoreResult<Self> {
        let delta: Delta = bincode::deserialize(bytes)
    ...
    pub fn reconstruct_clock(&self, clock_delta: &std::collections::HashMap<String, u64>) -> VectorClock {
    ...
    pub fn deserialize_orset_element<T: DeserializeOwned>(&self, element_bytes: &[u8]) -> StoreResult<T> {
        bincode::deserialize(element_bytes)
            .map_err(|e| StoreError::Deserialization(e.to_string()))
    }
    ...
    pub fn deserialize_ormap_value<V: DeserializeOwned>(&self, value_bytes: &[u8]) -> StoreResult<V> {
        bincode::deserialize(value_bytes)
            .map_err(|e| StoreError::Deserialization(e.to_string()))
    }
