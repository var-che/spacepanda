...
pub fn apply_remote_to_channel(channel: &mut Channel, remote_channel: &Channel) -> StoreResult<()> {
...
pub fn apply_remote_to_space(space: &mut Space, remote_space: &Space) -> StoreResult<()> {
    // Merge all CRDT fields
...
impl Default for RemoteContext {
    fn default() -> Self {
        RemoteContext { validate_signatures: true, check_causality: true }
    }
}
...
impl RemoteContext {
    ...
    pub fn no_validation() -> Self {
        RemoteContext { validate_signatures: false, check_causality: false }
    }
}
...
mod tests {
    ...
    use crate::core_store::model::{ChannelId, ChannelType, Timestamp, UserId};
    ...
    fn test_apply_remote_to_channel() {
        ...
        remote_channel
            .name
            .set("Updated".to_string(), ts + 1000, "node2".to_string(), vc);
