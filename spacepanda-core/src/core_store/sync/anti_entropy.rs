...
impl AntiEntropyManager {
    ...
    pub fn add_peer(&mut self, peer_id: String, vector_clock: VectorClock) {
        ...
        self.peers.insert(
            peer_id.clone(),
            PeerSyncState { peer_id, vector_clock, last_sync: now, failure_count: 0 },
        );
    }
    ...
    pub fn select_peers_for_sync(&self) -> Vec<String> {
        ...
        peers.sort_by_key(|peer_id| self.peers.get(peer_id).map(|p| p.last_sync).unwrap_or(0));
...
mod tests {
    ...
    fn test_select_peers_for_sync() {
        let config = AntiEntropyConfig { max_peers_per_round: 2, ..Default::default() };
        let mut manager = AntiEntropyManager::new(config);
