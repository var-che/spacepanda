...
use crate::core_store::model::{Channel, Space, Message, MessageId, UserId, ChannelId, SpaceId, Timestamp};
...
impl QueryEngine {
    ...
    pub fn list_messages(
        ...
        let result: Vec<MessageInfo> = messages.iter()
            .skip(offset)
    ...
    pub fn get_user_role(&self, space_id: &SpaceId, user_id: &UserId) -> Option<String> {
        self.spaces.get(space_id)
            .and_then(|s| s.get_user_role_id(user_id))
            .cloned()
    }
}
