...
impl OpLog {
    ...
    pub fn get_by_node(&self, node_id: &str) -> Vec<&OpLogEntry> {
        self.entries
            .iter()
            .filter(|e| e.metadata.node_id == node_id)
            .collect()
    }
...
mod tests {
    ...
    fn test_oplog_append() {
        ...
        let op_id = log
            .append(vec![1, 2, 3], metadata, "test_op".to_string())
            .unwrap();
    ...
    fn test_oplog_get_since() {
        ...
        for i in 1..=5 {
            ...
            log.append(vec![i], metadata, "test_op".to_string())
                .unwrap();
        }
    ...
    fn test_oplog_merge() {
        ...
        log1.append(vec![1], metadata1, "test_op".to_string())
            .unwrap();
        
        // Add to log2
        vc.increment("node2");
        let metadata2 = OperationMetadata::new("node2".to_string(), vc);
        log2.append(vec![2], metadata2, "test_op".to_string())
            .unwrap();
    ...
    fn test_oplog_vector_clock_tracking() {
        ...
        vc.increment("node1");
        let metadata = OperationMetadata::new("node1".to_string(), vc.clone());
        log.append(vec![1], metadata, "test_op".to_string())
            .unwrap();
