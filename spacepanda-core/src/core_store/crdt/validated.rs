...
impl<C: Crdt> ValidatedCrdt<C> {
    ...
    fn verify_operation_signature(
        ...
        if !verified && (self.config.require_signatures || metadata.is_signed()) {
            return Err(StoreError::InvalidSignature(
                "Operation signature verification failed - not signed by any authorized key".to_string()
            ));
        }
    ...
    C::Operation: Serialize + HasMetadata,
{
    ...
    fn apply(&mut self, op: Self::Operation) -> StoreResult<()> {
        ...
        let operation_data = bincode::serialize(&op)
            .map_err(|e| StoreError::Serialization(format!("Failed to serialize operation: {}", e)))?;
...
mod tests {
    use super::*;
    use crate::core_store::crdt::or_set::{ORSet, ORSetOperation, AddId};
