...
impl OperationMetadata {
    pub fn new(node_id: String, vector_clock: VectorClock) -> Self {
        ...
        OperationMetadata { node_id, vector_clock, timestamp, signature: None }
    }
    ...
    pub fn verify_signature(
        ...
        match &self.signature {
            Some(sig_bytes) => {
                ...
                let public_key = PublicKey::from_bytes(public_key.to_vec()).map_err(|e| {
                    crate::core_store::store::errors::StoreError::InvalidSignature(e)
                })?;
...
mod tests {
    ...
    fn test_operation_metadata_with_signature() {
        let vc = VectorClock::new();
        let sig = vec![1, 2, 3, 4];
        let metadata = OperationMetadata::new("node1".to_string(), vc).with_signature(sig.clone());
    ...
    fn test_signature_verification_valid() {
        ...
        let vc = VectorClock::new();
        let metadata = OperationMetadata::new("node1".to_string(), vc).with_signature(signature.0);

        // Verify signature
        let result =
            metadata.verify_signature(operation_data, keypair.public_key(), "channel_123", true);
    ...
    fn test_signature_verification_forged() {
        ...
        let vc = VectorClock::new();
        let metadata = OperationMetadata::new("node1".to_string(), vc).with_signature(signature);

        // Try to verify with keypair2's public key (should fail)
        let result =
            metadata.verify_signature(operation_data, keypair2.public_key(), "channel_123", true);
    ...
    fn test_signature_verification_unsigned_required() {
        ...
        let result = metadata.verify_signature(operation_data, &dummy_pubkey, "channel_123", true);
    ...
    fn test_signature_verification_unsigned_optional() {
        ...
        let result = metadata.verify_signature(operation_data, &dummy_pubkey, "channel_123", false);
