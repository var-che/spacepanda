...
pub enum ORMapOperation<K: Clone, V: Clone> {
    ...
    Put { key: K, value: V, add_id: AddId, metadata: OperationMetadata },
    /// Remove a key
    Remove { key: K, metadata: OperationMetadata },
}
...
impl<K: Clone + Eq + std::hash::Hash, V: Clone> ORMap<K, V> {
    ...
    pub fn new() -> Self {
        ORMap { map: HashMap::new(), vector_clock: VectorClock::new() }
    }
...
impl<K: Clone + Eq + std::hash::Hash + Send + Sync, V: Clone + Send + Sync> Crdt for ORMap<K, V> {
    ...
    fn apply(&mut self, op: Self::Operation) -> StoreResult<()> {
        match op {
            ORMapOperation::Put { key, value, add_id, metadata } => {
...
mod tests {
    ...
    fn test_or_map_apply_remove() {
        ...
        let metadata = OperationMetadata::new("node1".to_string(), vc);
        let op = ORMapOperation::Remove { key: "test".to_string(), metadata };
