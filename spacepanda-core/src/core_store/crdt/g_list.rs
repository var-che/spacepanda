...
pub enum GListOperation<T: Clone> {
    ...
    Insert { id: ElementId, value: T, after: Option<ElementId>, metadata: OperationMetadata },
    /// Delete (tombstone) an element
    Delete { id: ElementId, metadata: OperationMetadata },
}
...
impl<T: Clone> GList<T> {
    ...
    pub fn new() -> Self {
        GList { elements: HashMap::new(), vector_clock: VectorClock::new() }
    }
    ...
    pub fn insert(&mut self, id: ElementId, value: T, after: Option<ElementId>, vc: VectorClock) {
        let element = Element { id: id.clone(), value, after, tombstone: false };
    ...
    fn ordered_elements(&self) -> Vec<&Element<T>> {
        ...
        let mut remaining: HashMap<ElementId, &Element<T>> =
            self.elements.iter().map(|(k, v)| (k.clone(), v)).collect();

        // Start with elements that have no predecessor (after = None)
        let mut to_process: Vec<&Element<T>> =
            self.elements.values().filter(|e| e.after.is_none()).collect();
        ...
        while let Some(current) = to_process.pop() {
            ...
            let mut next: Vec<&Element<T>> = remaining
                .values()
    ...
    pub fn get(&self, id: &ElementId) -> Option<&T> {
        self.elements.get(id).filter(|e| !e.tombstone).map(|e| &e.value)
    }

    /// Get the last element ID (for appending)
    pub fn last_id(&self) -> Option<ElementId> {
        self.ordered_elements().last().map(|e| e.id.clone())
    }
}
