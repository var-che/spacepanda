...
use crate::{
    ...
    core_store::{
        model::types::{ChannelId, SpaceId, UserId},
...
async fn test_offline_member_catches_up() -> anyhow::Result<()> {
    ...
    let charlie_msg =
        charlie_manager.send_message(&channel_id, b"I'm back! What did I miss?").await?;
    ...
    println!(
        "✅ Offline member successfully synced {} missed messages",
        missed_messages.len()
    );
