...
fn bench_ed25519_keypair_generation(c: &mut Criterion) {
    ...
    for batch_size in [10, 50, 100, 500].iter() {
        ...
        group.bench_with_input(
            BenchmarkId::new("batch_generation", batch_size),
            batch_size,
            |b, &n| {
                b.iter(|| {
                    let keys: Vec<SigningKey> =
                        (0..n).map(|_| deterministic_signing_key(&mut rng)).collect();
                    black_box(keys)
                });
            },
        );
...
fn bench_x25519_key_exchange(c: &mut Criterion) {
    ...
    for batch_size in [10, 50, 100, 500].iter() {
        group.throughput(Throughput::Elements(*batch_size as u64));
        group.bench_with_input(
            BenchmarkId::new("batch_exchanges", batch_size),
            batch_size,
            |b, &n| {
                b.iter(|| {
...
fn bench_noise_handshake(c: &mut Criterion) {
    ...
    group.bench_function("full_handshake", |b| {
        b.iter(|| {
            // Initiator
            let mut initiator = Builder::new(params.parse().unwrap()).build_initiator().unwrap();

            // Responder
            let mut responder = Builder::new(params.parse().unwrap()).build_responder().unwrap();
...
fn bench_hkdf_key_derivation(c: &mut Criterion) {
    ...
    for num_keys in [10, 50, 100, 500].iter() {
        ...
        group.bench_with_input(
            BenchmarkId::new("batch_derivation", num_keys),
            num_keys,
            |b, &n| {
                b.iter(|| {
