...
fn bench_channel_metadata_operations(c: &mut Criterion) {
    ...
    group.bench_function("save_channel_metadata", |b| {
        ...
        b.to_async(&runtime).iter(|| {
            ...
            async move {
                ...
                storage.save_channel_metadata(&group_id, name, topic, members, 1).await.unwrap();
    ...
    for size in [10, 100, 1000, 10_000].iter() {
        ...
        group.bench_with_input(BenchmarkId::new("save_varying_size", size), size, |b, _| {
            let runtime = tokio::runtime::Runtime::new().unwrap();
...
fn bench_encryption_overhead(c: &mut Criterion) {
    use spacepanda_core::core_mls::storage::metadata_encryption::{
        decrypt_metadata, encrypt_metadata,
    };
