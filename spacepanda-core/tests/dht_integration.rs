...
use spacepanda_core::core_dht::{
    DhtKey, DhtValue, DhtStorage, RoutingTable, DhtConfig,
...
async fn test_local_store_and_retrieve() {
    ...
    for i in 0..5 {
        ...
        let value = DhtValue::new(format!("value_{}", i).as_bytes().to_vec())
            .with_ttl(3600);
...
async fn test_concurrent_storage_operations() {
    ...
    for i in 0..10 {
        ...
        let handle = tokio::spawn(async move {
            ...
            let value = DhtValue::new(format!("concurrent_value_{}", i).as_bytes().to_vec())
                .with_ttl(3600);
...
async fn test_storage_cleanup() {
    ...
    node.store_local(key1, DhtValue::new(b"value1".to_vec()).with_ttl(3600)).await.unwrap();
    node.store_local(key2, DhtValue::new(b"value2".to_vec()).with_ttl(0)).await.unwrap();
    node.store_local(key3, DhtValue::new(b"value3".to_vec()).with_ttl(0)).await.unwrap();
...
async fn test_sequence_number_conflict_resolution() {
    ...
    let value2 = DhtValue::new(b"value2".to_vec())
        .with_ttl(3600)
        .with_sequence(2);
    ...
    for i in 0..20 {
        ...
        let handle = tokio::spawn(async move {
            ...
            if i % 2 == 0 {
                ...
                let value = DhtValue::new(format!("value_{}", i / 2).as_bytes().to_vec())
                    .with_ttl(3600);
                node_clone.store_local(key, value).await
            } else {
...
impl OnionDhtNode {
    ...
    async fn send_dht_message_anonymous(&self, destination: PeerId, message: DhtMessage) -> Result<(), String> {
...
async fn test_multiple_anonymous_operations() {
    ...
    for i in 0..5 {
        ...
        let value = DhtValue::new(format!("private_value_{}", i).as_bytes().to_vec())
            .with_ttl(3600);
