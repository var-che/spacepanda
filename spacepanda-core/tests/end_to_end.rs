...
use spacepanda_core::core_identity::{GlobalIdentity, DeviceId, Keypair, KeyType};
use spacepanda_core::core_store::model::{Space, Channel, SpaceId, ChannelId, UserId, Timestamp, ChannelType};
use spacepanda_core::core_store::store::{LocalStore, LocalStoreConfig};
use spacepanda_core::core_dht::{DhtStorage, DhtKey, DhtValue, RoutingTable, PeerContact};
...
async fn test_end_to_end_multi_peer_data_sync() {
    ...
    let space_dht_value = DhtValue::new(space_data)
        .with_owner(alice_dht_id)
        .with_ttl(3600);
    ...
    let channel_dht_value = DhtValue::new(channel_data)
        .with_owner(alice_dht_id)
        .with_ttl(3600);
    ...
    let discovered_channel: Channel = serde_json::from_slice(&discovered_channel_value.data).unwrap();
    ...
    assert_eq!(
        alice_space_check.name.get().unwrap(),
        bob_space_check.name.get().unwrap()
    );
    ...
    assert_eq!(
        alice_channel_check.name.get().unwrap(),
        bob_channel_check.name.get().unwrap()
    );
