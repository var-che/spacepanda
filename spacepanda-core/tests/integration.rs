...
use spacepanda_core::core_identity::{GlobalIdentity, DeviceId, Keypair, KeyType, DeviceMetadata};
...
use spacepanda_core::core_dht::{DhtStorage, DhtKey, DhtValue, RoutingTable};
...
async fn test_integration_dht_routing_storage() {
    ...
    let peer_keys: Vec<_> = (0..10)
        .map(|i| {
            ...
            let peer = spacepanda_core::core_dht::PeerContact::new(
                peer_id,
                format!("192.0.2.{}:1234", i)
            );
