...
impl TestActor {
    ...
    async fn new(name: &str) -> Result<Self> {
        ...
        // Create manager
        let manager = Arc::new(ChannelManager::new(mls_service, store, identity.clone(), config));

        // Generate KeyPackage for this actor (needed before receiving invites)
        let key_package = manager.generate_key_package().await?;

        Ok(Self { name: name.to_string(), identity, manager, key_package, data_dir })
    }
...
async fn test_member_removal_during_simulated_disconnection() -> Result<()> {
    ...
    println!("✅ All remaining members can communicate");
    println!(
        "✅ Removal during disconnection works (network sync not tested in isolated environment)"
    );
...
async fn test_promote_demote_permissions() -> Result<()> {
    ...
    let charlie_promote_result =
        charlie.promote_to_admin(&channel_id, &diana.identity.user_id).await;
    assert!(
        charlie_promote_result.is_err(),
        "Regular member should still not be able to promote"
    );
