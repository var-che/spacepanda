            ...
            while let Some(event) = router_clone.next_event().await {
                match event {
                    RouterEvent::DataReceived(peer_id, data) => {
                        println!("[{}] Received {} bytes from peer {:?}", name_clone, data.len(), peer_id);
...
async fn test_network_commit_propagation() -> Result<()> {
    ...
    alice.register_peer(&channel_id, alice.identity.user_id.clone(), alice.get_peer_id()).await;
    alice.register_peer(&channel_id, bob.identity.user_id.clone(), bob.get_peer_id()).await;
    alice.register_peer(&channel_id, charlie.identity.user_id.clone(), charlie.get_peer_id()).await;
    
    // Register all peers in Bob's network
    bob.register_peer(&channel_id, alice.identity.user_id.clone(), alice.get_peer_id()).await;
    bob.register_peer(&channel_id, bob.identity.user_id.clone(), bob.get_peer_id()).await;
    bob.register_peer(&channel_id, charlie.identity.user_id.clone(), charlie.get_peer_id()).await;
    
    // Register all peers in Charlie's network
    charlie.register_peer(&channel_id, alice.identity.user_id.clone(), alice.get_peer_id()).await;
    charlie.register_peer(&channel_id, bob.identity.user_id.clone(), bob.get_peer_id()).await;
    charlie.register_peer(&channel_id, charlie.identity.user_id.clone(), charlie.get_peer_id()).await;
...
async fn test_removal_with_network_broadcast() -> Result<()> {
    ...
    for actor in [&alice, &bob, &charlie] {
        actor.register_peer(&channel_id, alice.identity.user_id.clone(), alice.get_peer_id()).await;
        actor.register_peer(&channel_id, bob.identity.user_id.clone(), bob.get_peer_id()).await;
        actor.register_peer(&channel_id, charlie.identity.user_id.clone(), charlie.get_peer_id()).await;
    }
